 ...
 * `use-visibility-options.ts` never selects this implementation and Rollup
 * drops it from the bundle. The module exists only so the import graph
 * resolves.
 *
 * Without the add-on a topic can only be public, so the free implementation
 * already describes the behaviour exactly.
 */
export { default } from './use-visibility-options.free'
