...
/** Single brand blue, shared with the portal. Plain CSS reads --bc-brand. */
const BRAND = '#3266ea'
...
/**
 * Button tokens for the admin panel.
 *
 * Deliberately keeps antd's default control heights, unlike the portal: these
 * screens pair buttons with inputs inside Input.Search and Space.Compact
 * groups, where a taller button alone would break the seam between the two.
 * Everything that does not affect the control box — weight, shadow, hover
 * surface — is aligned with the portal so the two apps feel like one product.
 */
export function buttonTheme(isDark: boolean) {
  return {
    ...
    // 400 reads like a label; 500 reads like an action.
    fontWeight: 500,

    paddingInline: 15,
    paddingInlineSM: 11,

    // Flat surfaces everywhere else in the panel — solid buttons should match.
    dangerShadow: 'none',
    defaultShadow: 'none',
    primaryShadow: 'none',
