...
/**
 * Props for the per-member capability popover.
 *
 * Declared here rather than in either sibling. Both editions implement this
 * control, so the contract belongs to neither implementation — and the free
...
/** Props for the badge catalog modal. Owned here for the reason above. */
export interface ProfileBadgesModalProps {
  onClose: () => void
  open: boolean
}

/** Props for the per-member badge assignment popover. Owned here likewise. */
